...
export function requestForecast() {
    return { type: GET_FORECAST_REQUESTED };
}

export function fetchForecastSuccess({ response }) {
    console.log(response);
    const { list } = response;

    return action(
        GET_FORECAST_SUCCESS,
        fromJS({ forecast: new List(list.map(day => fromJS(day))) })
    );
}

export function fetchForecastEpic(action$) {
    return action$.ofType(GET_FORECAST_REQUESTED)
        .mergeMap(() => {
            return ajax({
                crossDomain: true,
                method: 'GET',
                url: getURI('forecast'),
            }).map(response => fetchForecastSuccess(response))
                .catch(error => {
                    console.log(error);
                    return Observable.of(action(
                        GET_FORECAST_FAILURE,
                        new Map({ forecast: new List(error.xjr.response) }
                    )));
                });
        });
}
